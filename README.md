...
This repository ships inference-only code, but the [`training/`](training/) directory contains a
community-built [PyTorch Lightning](https://lightning.ai/) pipeline for fine-tuning (or retraining)
the FlowLM backbone on your own data, following the recipe from the
[technical report](https://arxiv.org/abs/2509.06926) (flow-matching + Lagrangian Self-Distillation
losses, Table 14 hyperparameters).

**Prerequisite: gated weights.** The public checkpoints (`kyutai/pocket-tts-without-voice-cloning`)
ship with a zeroed-out Mimi encoder and cannot be used to extract training latents from audio. To
fine-tune, accept the terms at [huggingface.co/kyutai/pocket-tts](https://huggingface.co/kyutai/pocket-tts)
and log in locally:
...
```bash
pip install -e ".[dev]" lightning "datasets[audio]" soundfile

# Unit tests (no network / weights needed)
pytest training/tests -v

# Fine-tune on a local LJSpeech-format dataset
python -m training.train fit --config training/configs/brspeech_validate.yaml

# Export a checkpoint back to a stock-CLI-compatible config
python -m training.export <checkpoint>.ckpt <out_dir>
pocket-tts generate --config <out_dir>/config.yaml --text "Hello world"
```

See [`training/README.md`](training/README.md) for the full setup, dataset formats (local LJSpeech
and Hugging Face streaming), what's trained vs. frozen (the Mimi VAE stays frozen), and known caveats
(VRAM budgeting, EOS-head calibration, licensing of third-party datasets).
